...
async def _can_configure(update: Update) -> bool:
    """Owner or authorized admin can configure."""
    ...
    if not ok:
        await update.message.reply_text("⛔ Only the owner or authorized admins can use this command.")
        return False
    return True

async def _owner_only(update: Update) -> bool:
    """Strict owner-only (for addadmin/removeadmin)."""
    if not update.effective_user or not update.message:
        return False
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if authorized:
        ...
        if user.id == OWNER_ID:
            text += "\n\n*Owner only:* `/addadmin` `/removeadmin` `/listadmins`"
...
async def addadmin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not context.args:
        await update.message.reply_text(
            ...
            "User must send /myid to the bot to get their numeric ID.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return
    try:
    ...
    factory = get_session_factory()
    async with factory() as session:
        added = await add_admin(session, target_id, update.effective_user.id)
        await session.commit()
    if added:
        await update.message.reply_text(
            f"✅ User `{target_id}` can now configure the bot.",
            parse_mode=ParseMode.MARKDOWN,
        )
        logger.info("Admin added: %s by %s", target_id, update.effective_user.id)
    else:
        await update.message.reply_text(
...
async def removeadmin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if removed:
        await update.message.reply_text(
            f"✅ Removed admin rights from `{target_id}`.",
            parse_mode=ParseMode.MARKDOWN,
        )
    else:
        await update.message.reply_text(
            f"⚠️ `{target_id}` was not an admin.", parse_mode=ParseMode.MARKDOWN
        )
...
async def listadmins_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if admins:
        lines.append("*Authorized admins:*")
        for a in admins:
...
# Per-chat locks: process messages in arrival order (no size-based reordering)
...
async def process_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Process text/caption edits and document filename renames.

    Order: messages in the same chat are handled one-by-one (FIFO),
    so smaller files do not jump ahead of larger ones.
    """
    ...
    # Serialize per chat so upload order == arrival order
    lock = _get_chat_lock(chat.id)
    async with lock:
        await _process_one_message(
            update, context, message, chat, rules, case_sensitive, match_mode
        )
...
async def _process_one_message(
    ...
    # ----- Document filename rename (delete + re-upload) -----
    ...
    if doc and doc.file_name:
        ...
        if name_changed:
            ...
            return  # original deleted; do not also edit caption on old msg

    # ----- Normal text / caption in-place edit -----
    if not text_changed:
        return
...
async def _rename_document(
    ...
    """
    Download document → re-upload with new filename → delete original.

    Uses in-memory bytes (no temp file) to avoid corrupting Hindi / Unicode names.
    Filename is NFC-normalized so Devanagari matras stay intact.
    """
    doc = message.document
    if not doc:
        return

    # Keep Unicode filename stable (Hindi matras etc.)
    new_filename = unicodedata.normalize("NFC", new_filename or "file")
    if new_caption:
        new_caption = unicodedata.normalize("NFC", new_caption)

    try:
        # 1) Download fully into memory (avoids filesystem encoding issues)
        tg_file = await context.bot.get_file(doc.file_id)
        data = await tg_file.download_as_bytearray()
        bio = BytesIO(bytes(data))
        bio.name = new_filename  # some clients read name from the buffer

        # 2) Re-upload with explicit Unicode filename
        ...
        # 3) Delete original only after successful upload
        try:
            ...
            logger.warning(
                "Re-uploaded renamed file but could not delete original %s: %s",
                message.message_id,
                del_err,
            )

        logger.info(
            "Renamed document msg %s in %s: %r → %r",
            message.message_id,
            chat_id,
            doc.file_name,
            new_filename,
        ...
        logger.warning("Flood control during rename: retry after %s s", e.retry_after)
    except Forbidden as e:
        logger.warning("Forbidden during document rename in %s: %s", chat_id, e)
    except TelegramError as e:
        logger.error("Telegram error during document rename: %s", e)
    except Exception as e:
        logger.exception("Failed to rename document: %s", e)
