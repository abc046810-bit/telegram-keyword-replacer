...
async def _rename_document(
    ...
    Keeps message position roughly correct by doing this under the chat lock
    in arrival order.
    """
    doc = message.document
    if not doc:
    ...
    try:
        ...
        suffix = FsPath(doc.file_name or "file").suffix or ""
        fd, tmp_path = tempfile.mkstemp(suffix=suffix)
        os.close(fd)
        await tg_file.download_to_drive(custom_path=tmp_path)

        # 2) Re-upload with new name (and updated caption if any)
        with open(tmp_path, "rb") as f:
            await context.bot.send_document(
                chat_id=chat_id,
                document=InputFile(f, filename=new_filename),
        ...
        logger.info(
            "Renamed document msg %s in %s: %s → %s",
            message.message_id,
            chat_id,
            doc.file_name,
            new_filename,
